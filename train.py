...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size):
        ...
        # Quiet attention (softmax denominator +1): y_quiet = y * D/(D+1), D=exp(lse).
        # lse is (B, H, T) log-sum-exp of scaled scores; sigmoid(lse) = D/(D+1).
        y = y * torch.sigmoid(lse.transpose(1, 2).unsqueeze(-1)).to(y.dtype)
