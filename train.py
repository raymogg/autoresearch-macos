...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        # U-Net-style cross-depth skip connections (modded-nanogpt lineage):
        # first half = encoder, second half = decoder, learnable scalar gains.
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        param_groups = [
            ...
            dict(kind='adamw', params=skip_params, lr=scalar_lr, betas=adam_betas, eps=1e-10, weight_decay=0.0),
        ]
    ...
    def forward(self, idx, targets=None, reduction='mean'):
        ...
        x = norm(x)
        x0 = x
        n_encoder = self.config.n_layer // 2
        skip_connections = []
        for i, block in enumerate(self.transformer.h):
            x = self.resid_lambdas[i] * x + self.x0_lambdas[i] * x0
            if i >= n_encoder:
                x = x + self.skip_weights[i - n_encoder] * skip_connections.pop()
