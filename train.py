...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size, attn_scale):
        B, T, C = x.size()
        ...
        # Learnable per-layer attention temperature: fold the scale into q so it
        # is differentiable (FA3's softmax_scale arg is a static float). At init
        # attn_scale=0.12, so this is numerically identical to softmax_scale=0.12.
...
class Block(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size, attn_scale):
        x = x + self.attn(norm(x), ve, cos_sin, window_size, attn_scale)
        x = x + self.mlp(norm(x))
        return x
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.attn_scales = nn.Parameter(torch.full((config.n_layer,), 0.12))
        # Value embeddings
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        attn_scale_params = [self.attn_scales]
        ...
        param_groups = [
            ...
            dict(kind='adamw', params=attn_scale_params, lr=scalar_lr * 0.01, betas=adam_betas, eps=1e-10, weight_decay=0.0),
        ]
    ...
    def forward(self, idx, targets=None, reduction='mean'):
        ...
        for i, block in enumerate(self.transformer.h):
            ...
            x = block(x, ve, cos_sin, self.window_sizes[i], self.attn_scales[i])
        x = norm(x)
