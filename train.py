...
class GPT(nn.Module):
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        lm_head_bias_params = [self.lm_head_bias]
        resid_params = [self.resid_lambdas]
        x0_params = [self.x0_lambdas]
        assert len(list(self.parameters())) == (len(matrix_params) + len(embedding_params) +
            len(lm_head_params) + len(lm_head_bias_params) + len(value_embeds_params) +
        ...
        param_groups = [
            ...
            dict(kind='adamw', params=lm_head_bias_params, lr=lm_head_bias_lr * dmodel_lr_scale, betas=adam_betas, eps=1e-10, weight_decay=0.0),
